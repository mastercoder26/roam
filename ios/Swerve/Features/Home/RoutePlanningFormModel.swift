...
enum RoutePlanningFormReducer {
    static func applying(
        ...
        if route.waypointCount > 0 {
            extraStopsMessage = " This shared route has \(route.waypointCount) additional stop\(route.waypointCount == 1 ? "" : "s"). Swerve will analyze the start and final destination."
        } else {
            extraStopsMessage = ""
        }
        ...
        return RoutePlanningFormState(
            ...
            importNotice: RouteImportNotice(
                ...
                message: "Review the route, then analyze it. Swerve recalculates a driving route from these locations.\(extraStopsMessage)",
                isError: false
            )
        )
    }
