...
struct DriverProgressView: View {
    ...
    private var emptyEvidenceState: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("No recorded evidence yet")
                .font(.headline)
            Text("Complete a drive with usable GPS and motion. It stays in history while Swerve automatically analyzes its start-to-destination route difficulty.")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var measurementNote: some View {
        Label(
            "Swerve keeps driving history, route overlap, and progress calculations on this device.",
